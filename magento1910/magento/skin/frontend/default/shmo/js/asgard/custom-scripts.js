...
function animatePage(selector, animation){
	...
	//animate opacity and direction according to the animation given
	switch(animation){
		...
		case "all":
			var counter = 0;
			selector.delay(800).animate({"opacity":"1"},{duration:animationLength, easing:"easeInOutQuad"});

			selector.find("div *:not(div, li, li a, td)").each(function(){
		...
		case "half":
			selector.children("div.half-column").css({"position":"relative", "top":"-80px"});
			selector.children("div.half-column.last").css({"position":"relative", "top":"80px"});

			selector.delay(800).animate({"opacity":"1"},{duration:animationLength, easing:"easeInOutQuad"});
			selector.children("div.half-column").delay(800).animate({"top":"0px"},{duration:animationLength, easing:"easeInOutQuad", complete:function(){$(this).css({"top":"", "opacity":"", "position":""});}});
			selector.children("div.half-column.last").delay(800).animate({"top":"0px"},{duration:animationLength, easing:"easeInOutQuad", complete:function(){$(this).css({"top":"", "opacity":"", "position":""});}});
			break;

		case "top":
			selector.css({"position":"relative", "top":"-80px"});
			selector.delay(800).animate({"opacity":"1", "top":"0px"},{duration:animationLength, easing:"easeInOutQuad"});
			break;

		case "left":
			selector.css({"position":"relative", "left":"-80px"});
			selector.delay(800).animate({"opacity":"1", "left":"0px"},{duration:animationLength, easing:"easeInOutQuad"});
			break;

		case "right":
			selector.css({"position":"relative", "right":"-80px"});
			selector.delay(800).animate({"opacity":"1", "right":"0px"},{duration:animationLength, easing:"easeInOutQuad"});
			break;

		case "bottom":
			selector.css({"position":"relative", "bottom":"-80px"});
			selector.delay(800).animate({"opacity":"1", "bottom":"0px"},{duration:animationLength, easing:"easeInOutQuad"});
			break;
	...
	$("#hideMenu").on("click", function(){
		...
		if(windowWidth <= 960){
			...
			if(menuVisible == true){
				...
				$("#main-wrapper").animate({"left":"0px", "width":windowWidth+"px"},{duration:animationLength, easing:"easeInOutQuad"});
				$("#menu-wrapper").animate({"left":-menuWidth+"px"},{duration:animationLength, easing:"easeInOutQuad", complete:function(){
				...
				$("#main-wrapper").animate({"left":menuWidth+"px"},{duration:animationLength, easing:"easeInOutQuad"});
				$("#menu-wrapper").animate({"left":"0px"},{duration:animationLength, easing:"easeInOutQuad", complete:function(){
...
function animateImages(){
	...
	$(".masonryImage").each(function(){
		...
		$(this).on("mouseenter", function(evt){
			...
			hoverItem.stop().css("width","100%").animate({"opacity":"1", "left":"0px", "top":"0px", "height":thisHeight}, {duration:animationLength, easing:"easeInOutQuad"});
...
function responsive(){
	...
	if(windowWidth<=960){
		...
		if(menuVisible)
			$("#main-wrapper").css({"top":menuWrapperHeight + 5 +"px", "height":windowHeight - menuWrapperHeight - 5 +"px", "width":"100%", "left":"0px"});
		else
			$("#main-wrapper").css({"top":"5px", "height":windowHeight - 5 + "px", "width":"100%", "left":"0px"});

		$("#menu-wrapper").css({"left":"0px"});

		$("#social-wrapper, #menu").css({"display": "none", "opacity":"0"});
		$("#tablet-menu").css("display", "block");

		//change to the small logo
		$("#logo").attr("src", "images/logo-tablet.png");
		...
		//if menu is visible or not, adjust
		if(menuVisible)
			$("#main-wrapper").css({"left":menuWrapperWidth + 5 +"px", "width":windowWidth - menuWrapperWidth - 5 +"px", "height":"100%", "top":"0px"});
		else
			$("#main-wrapper").css({"left":"5px", "width":windowWidth - 5 + "px", "height":"100%", "top":"0px"});

		$("#menu-wrapper").css({"top":"0px"});


		$("#social-wrapper, #menu").css({"display": "block", "opacity":"1"});
		$("#tablet-menu").css("display", "none");

		//change to the normal logo
...
//function that handles the masonry portfolio layout
function masonryPortfolio(){

	var $container = $('#masonryPortfolio');
...
function innerImagesLoaded(callback){
    ...
    $(document).ready(function(){
    	...
    	if(images.length > 0)
	    	...
	    	setTimeout(function(){$("#percentage").html("0%");}, animationLength/2);
	    	callback();
...
jQuery.fn.center = function () {
	...
	this.css("left", ((document.documentElement.clientWidth - this.outerWidth()) / 2) + "px");
	return this;
}
...
function animationFinish(){
	...
	if(windowWidth <= 960){
		...
		//animate the menu and main wrappers
		$("#menu-wrapper").delay(400).animate({"opacity":"1", "left":"0px"},{duration:animationLength, easing: "easeInOutQuad"});
		$("#main-wrapper").delay(800).animate({"left":menuWrapperWidth + 5 + "px"},{duration:animationLength, easing: "easeInOutQuad", complete:function(){
